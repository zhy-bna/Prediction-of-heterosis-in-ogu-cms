...
# SVM with CV and external test
run_svm <- function(X_tr, y_tr, X_te, y_te, seed) {
  valid_tr <- !is.na(y_tr)
  ...
  for (i in seq_along(folds)) {
    ...
    m <- ksvm(X_tr_fold, y_tr[train_idx], kernel = svm_kernel, C = svm_C, cache = 500)
  ...
  m_full <- ksvm(X_tr_all, y_tr, kernel = svm_kernel, C = svm_C, cache = 500)
...
predict_svm <- function(X_tr, y_tr, X_te) {
  valid <- !is.na(y_tr)
  ...
  scale_vec[scale_vec == 0] <- 1
  m <- ksvm(X_tr, y_tr, kernel = svm_kernel, C = svm_C, cache = 500)
...
for (seed in SEEDS) {
  ...
  for (trait in TRAITS) {
    ...
    for (g in unique(cms)) {
      ...
      pred_lomo[val] <- predict_svm(X_tr[-val, , drop = FALSE], y_tr[-val], X_tr[val, , drop = FALSE])
    }
    ...
    for (g in unique(restorer)) {
      ...
      if (length(val) == 0 || length(setdiff(seq_along(y_tr), val)) < 2) next
      pred_loro[val] <- predict_svm(X_tr[-val, , drop = FALSE], y_tr[-val], X_tr[val, , drop = FALSE])
    }
    ...
    if (!is.null(X_te_full) && !is.null(pheno_te[[trait]])) {
      ...
      y_te <- y_te[valid_te]
      svm_res <- run_svm(X_tr, y_tr, X_te, y_te, seed)
      r_cv <- svm_res$r_cv
      r_test <- svm_res$r_test
    } else {
      r_cv <- r_test <- NA
    }
