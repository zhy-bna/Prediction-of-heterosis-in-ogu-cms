...
run_bayesA <- function(X_tr, y_tr, X_te, y_te, seed) {
  valid_tr <- !is.na(y_tr)
  ...
  for (i in seq_along(folds)) {
    ...
    X_fold <- scale(X_tr[train_idx, ], center = TRUE, scale = TRUE)
    X_fold[is.nan(X_fold)] <- 0
    center <- attr(X_fold, "scaled:center")
    scale_vec <- attr(X_fold, "scaled:scale")
    scale_vec[scale_vec == 0] <- 1
    m <- BGLR(y = y_tr[train_idx], ETA = list(list(X = X_fold, model = "BayesA")),
              nIter = nIter, burnIn = burnIn, thin = thin, verbose = FALSE)
    X_test_fold <- scale(X_tr[test_idx, ], center = center, scale = scale_vec)
    X_test_fold[is.nan(X_test_fold)] <- 0
    pred_cv[test_idx] <- as.numeric(m$mu) + X_test_fold %*% m$ETA[[1]]$b
  }
...
predict_bayesA <- function(X_tr, y_tr, X_te) {
  ...
  scale_vec[scale_vec == 0] <- 1
  m <- BGLR(y = y_tr, ETA = list(list(X = X_tr, model = "BayesA")),
            nIter = nIter, burnIn = burnIn, thin = thin, verbose = FALSE)
  X_te <- scale(X_te, center = center, scale = scale_vec)
  X_te[is.nan(X_te)] <- 0
  as.vector(as.numeric(m$mu) + X_te %*% m$ETA[[1]]$b)
}
...
for (seed in SEEDS) {
  ...
  for (trait in TRAITS) {
    ...
    for (g in unique(cms)) {
      ...
      pred_lomo[val] <- predict_bayesA(X_tr[-val, , drop = FALSE], y_tr[-val], X_tr[val, , drop = FALSE])
    }
    ...
    for (g in unique(restorer)) {
      ...
      if (length(val) == 0 || length(setdiff(seq_along(y_tr), val)) < 2) next
      pred_loro[val] <- predict_bayesA(X_tr[-val, , drop = FALSE], y_tr[-val], X_tr[val, , drop = FALSE])
    }
    ...
    if (!is.null(X_te_full) && !is.null(pheno_te[[trait]])) {
      ...
      y_te <- y_te[valid_te]
      bayes_res <- run_bayesA(X_tr, y_tr, X_te, y_te, seed)
      r_cv <- bayes_res$r_cv
      r_test <- bayes_res$r_test
    } else {
      r_cv <- r_test <- NA
    }
