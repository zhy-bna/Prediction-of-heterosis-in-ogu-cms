...
run_bayesB <- function(X_tr, y_tr, X_te, y_te, seed) {
  valid_tr <- !is.na(y_tr)
  ...
  for (i in seq_along(folds)) {
    ...
    m <- BGLR(y = y_tr[train_idx], ETA = list(list(X = X_fold, model = "BayesB")),
  ...
  m_full <- BGLR(y = y_tr, ETA = list(list(X = X_tr_all, model = "BayesB")),
                 nIter = nIter, burnIn = burnIn, thin = thin, verbose = FALSE)
...
predict_bayesB <- function(X_tr, y_tr, X_te) {
  valid <- !is.na(y_tr)
  ...
  scale_vec[scale_vec == 0] <- 1
  m <- BGLR(y = y_tr, ETA = list(list(X = X_tr, model = "BayesB")),
            nIter = nIter, burnIn = burnIn, thin = thin, verbose = FALSE)
...
for (seed in SEEDS) {
  ...
  for (trait in TRAITS) {
    ...
    for (g in unique(cms)) {
      ...
      pred_lomo[val] <- predict_bayesB(X_tr[-val, , drop = FALSE], y_tr[-val], X_tr[val, , drop = FALSE])
    }
    ...
    for (g in unique(restorer)) {
      ...
      if (length(val) == 0 || length(setdiff(seq_along(y_tr), val)) < 2) next
      pred_loro[val] <- predict_bayesB(X_tr[-val, , drop = FALSE], y_tr[-val], X_tr[val, , drop = FALSE])
    }
    ...
    if (!is.null(X_te_full) && !is.null(pheno_te[[trait]])) {
      ...
      y_te <- y_te[valid_te]
      bayes_res <- run_bayesB(X_tr, y_tr, X_te, y_te, seed)
      r_cv <- bayes_res$r_cv
