...
# RKHS with CV and external test
run_rkhs <- function(X_tr, y_tr, X_te, y_te, seed) {
  valid_tr <- !is.na(y_tr)
  ...
  for (i in seq_along(folds)) {
    ...
    m <- gausspr(X_tr_fold, y_tr[train_idx], kernel = rkhs_kernel, variance.model = FALSE)
    X_te_fold <- scale(X_tr[test_idx, ], center = center, scale = scale_vec)
    X_te_fold[is.nan(X_te_fold)] <- 0
    pred_cv[test_idx] <- predict(m, X_te_fold)
  }
  ...
  m_full <- gausspr(X_tr_all, y_tr, kernel = rkhs_kernel, variance.model = FALSE)
  X_te_all <- scale(X_te, center = center_all, scale = scale_all)
  X_te_all[is.nan(X_te_all)] <- 0
  pred_te <- predict(m_full, X_te_all)
  r_test <- cor(y_te, pred_te, use = "complete.obs")
  list(r_cv = r_cv, r_test = r_test)
}

# Predict for a given training set
predict_rkhs <- function(X_tr, y_tr, X_te) {
  valid <- !is.na(y_tr)
  ...
  scale_vec[scale_vec == 0] <- 1
  m <- gausspr(X_tr, y_tr, kernel = rkhs_kernel, variance.model = FALSE)
  X_te <- scale(X_te, center = center, scale = scale_vec)
  X_te[is.nan(X_te)] <- 0
  as.vector(predict(m, X_te))
}
...
for (seed in SEEDS) {
  ...
  for (trait in TRAITS) {
    ...
    for (g in unique(cms)) {
      ...
      pred_lomo[val] <- predict_rkhs(X_tr[-val, , drop = FALSE], y_tr[-val], X_tr[val, , drop = FALSE])
    }
    ...
    for (g in unique(restorer)) {
      ...
      if (length(val) == 0 || length(setdiff(seq_along(y_tr), val)) < 2) next
      pred_loro[val] <- predict_rkhs(X_tr[-val, , drop = FALSE], y_tr[-val], X_tr[val, , drop = FALSE])
    }
    ...
    if (!is.null(X_te_full) && !is.null(pheno_te[[trait]])) {
      ...
      y_te <- y_te[valid_te]
      rkhs_res <- run_rkhs(X_tr, y_tr, X_te, y_te, seed)
      r_cv <- rkhs_res$r_cv
      r_test <- rkhs_res$r_test
    } else {
      r_cv <- r_test <- NA
    }
